 ...
 * // and will be updatable 所述ResultSet接口提供getter方法（ getBoolean ， getLong ，用于从当前行检索列值，等等）。 可以使用列的索引号或列的名称来检索值。 一般来说，使用列索引将更有效率。 列从1编号。为了最大可移植性，每行中的结果集列应以从左到右的顺序读取，每列应只读一次。
 ...
 * 更新方法可以以两种方式使用：
 * <p>
 * 更新当前行中的列值。 在可滚动的ResultSet对象中，光标可以向前和向后移动到绝对位置或相对于当前行的位置。 下面的代码段更新NAME所述的第五行中列ResultSet对象rs ，然后使用方法updateRow来更新该数据的源表rs推导。
 * rs.absolute(5); // moves the cursor to the fifth row of rs
 * rs.updateString("NAME", "AINSWORTH"); // updates the
 * // NAME column of row 5 to be AINSWORTH
 * rs.updateRow(); // updates the row in the data source 将列值插入到插入行中。 可更新的ResultSet对象具有与其相关ResultSet的特殊行，用作构建要插入的行的暂存区域。 以下代码片段将光标移动到插入行，构建一个三列行，并将其插入到rs ，并使用方法insertRow进入数据源表。
 ...
 * rs.insertRow();
 * rs.moveToCurrentRow(); 甲ResultSet当对象将自动关闭Statement生成它对象被关闭时，重新执行，或用于检索从多个结果的序列中的下一结果。
 * <p>

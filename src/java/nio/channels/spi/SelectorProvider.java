 ...
 * @since 1.4
 */
//SelectorProvider:选择器和可选择通道的服务提供者类。此类中的所有方法都可以安全地被多个并发线程使用。
public abstract class SelectorProvider {
    ...
    private static boolean loadProviderFromProperty() {
        ...
        if (cn == null)
            return false;
        try {
            //使用反射加载Class字节码，返回一个Class对象。java.nio.channels.spi.SelectorProvider
            Class<?> c = Class.forName(cn, true,
                    ClassLoader.getSystemClassLoader());
            //使用反射机制，通过Class字节码对象来创建SelectorProvider实例对象
    ...
    //实际上是获取当前的SelectorProvider，没有则创建一个新的
